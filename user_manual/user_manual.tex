...
First, we need to prepare a \textit{target file}. This is a text file which lists the lab-space coordinates of all the points on our calibration target according to the format shown in Fig.~\ref{fig:target_file}. Prepare such a file and save it in the experiment's calibration folder.
...
\begin{table}
\centering
\caption{The \texttt{params\_file.yml} parameters for the calibration step. All paths to files are relative to the \texttt{workflow.py} script.}
...
For each of the calibration images we make a \textit{calibration points file}. This is a file which lists the coordinates of the calibration target points in both lab space and image space coordinates. These text files follow a simple tab-separated format as shown in Fig.~\ref{fig:calpointfile}. Therefore:
%
\begin{enumerate}
	\item Make a new text file. Name it, for example \texttt{cal\_points\_cam1}. Alternatively, copy the file from the \texttt{example/calibration} folder in MyPTV.
...
\begin{enumerate}
	\item The segmentation is run first on a single image by specifying \texttt{Number\_of\_images: 1}. To view the results, use \texttt{plot\_result: True}, and make sure the results are not saved by using \texttt{save\_name: None}.
	
	\item Once the parameters are set for the first image, set \texttt{Number\_of\_images: None} to loop over all the images in the folder, and save the results by setting \texttt{save\_name: /file/name/to/use}
\end{enumerate}
...
\begin{table}[!ht]
	...
	\begin{tabular}{l m{10cm}}
		...
		\texttt{Number\_of\_images} & Number of images over which to do the segmentation \\
		\texttt{images\_folder} & path to the folder containing the images\\
		
		\texttt{image\_extension} & extension of the images; for example, .tif\\
		...
		\texttt{plot\_result} & if \texttt{False} will not plot the results; if \texttt{True} the results will be plotted \\
		
		\texttt{threshold} & the brightness value for which a point is considered a particle after a local threshold is performed \\
		
		\texttt{blur\_sigma} & the standard deviation of a Gaussian blur filter \\
		 
		\texttt{local\_filter} & the size of a local mean subtraction filter \\
		
		\texttt{min\_xsize} & minimum particle size (pixels) in $x$ direction \\
		
		\texttt{min\_ysize} & minimum particle size (pixels) in $y$ direction \\
		
		\texttt{min\_area} & minimum particle area (pixels$^2$)\\
		
		\texttt{max\_xsize} & maximum particle size (pixels) in $x$ direction \\
		
		\texttt{max\_ysize} & maximum particle size (pixels) in $y$ direction \\
		
		\texttt{max\_area} & maximum particle area (pixels$^2$) \\
...
% Ron, learn: \verb|filename___with_many___underscores|

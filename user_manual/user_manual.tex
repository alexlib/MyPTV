...
The 3D model is a core element of the PTV method, as it allows to do the transformation between the coordinates of objects in the 3D world (so-called lab-space) and the 2D coordinates in the image space seen by the cameras (namely, pixel coordinates) and vise-versa. 
In MyPTV there are two such models that have been implemented thus far, each having certain advantages and disadvantages with respect to the other. Other PTV programs can also use other 3D models. The models implemented are introduced below.
...
The extended Zolof model is a nother approach to obtaining the transformation between 3D lab-space and 2D image space. 
Here, the transformation is inherently mathematical, namely, in its basic version it does not involve any physical assumption about how cameras perceive the world. Thus the projection of 3D coordinates into the 2D lab coordinates is simply achieved using interpolating functions
%
\begin{equation}
\left[ 
\begin{matrix}
...
\end{equation}
%
In MyPTV, $P_\eta$ and $P_\zeta$ are simply polynomial functions of the $\vec{r}=(x,y,z)$ coordinates: ....



The extended Zolof model is a later addition to MyPTV, and its allows the software users to 

...
Note: to perform easy stereo matching of points in images, use the manual matching GUI. See Section~\ref{sec:man_match} and Fig.~\ref{fig:man_match}.
...
\begin{table}[!ht]
	...
	\begin{tabular}{l m{10cm}}
		...
		\hline
		%
		\texttt{cameras} & a list that holds the names of the cameras used; for example [cam1, cam2, cam3]. Note that the camera files need to be in the same folder where we are running the workflow script\\
		%
		\texttt{images} & the paths of the images from which matching should be performed. \\
		\hline
	\end{tabular}
\end{table}
